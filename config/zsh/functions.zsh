...
wpRecon() {
  echo "${yellow}[+] WP Recon ${reset}"
  cat ALLHTTP | nuclei -t $HOME/nuclei-wordfence-cve/nuclei-templates -o wordpress
  [ -s "wordpress" ] && echo "Wordpress vector found :)" | notify -silent -id wordpress
  [ -s "wordpress" ] && notify -silent -bulk -data wordpress -id wordpress
}
...
bruteTop1million() {
  wordlist="$HOME/Lists/subdomains-top1million-110000.txt"
  resolvers="$HOME/Lists/resolvers.txt"
  outfile="brutesubs_out.txt"
  cleanfile="clean.subdomains"
  alivefile="alive.subdomains"

  echo "[*] Iniciado brute force de subdomínios com shuffledns..."

  # Verificações iniciais
  [ ! -f "$wordlist" ] && echo "[x] Wordlist não encontrada." && return 1
  [ ! -f "$resolvers" ] && echo "[x] Resolvers não encontrado." && return 1
  [ ! -f domains ] && echo "[x] Arquivo 'domains' não encontrado." && return 1

  echo "[*] Limpando arquivos de saída..."
  truncate -s 0 "$outfile"
  truncate -s 0 "$cleanfile"
  truncate -s 0 "$alivefile"

  echo "[*] Iniciando brute force por domínio..."
  while read -r domain; do
    [ -z "$domain" ] && continue
    echo "[*] Brute forcing: $domain"
    shuffledns -d "$domain" -r "$resolvers" -w "$wordlist" -mode bruteforce -t 50 -o tmp.out

    if [ -s tmp.out ]; then
      anew "$outfile" < tmp.out | anew "$cleanfile"
      echo "[+] Subdomínios encontrados para $domain"
    else
      echo "[-] Nenhum subdomínio encontrado para $domain"
    fi
  ...
  httpx -l "$cleanfile" -silent -timeout 10 -status-code -no-color | anew "$alivefile"

  echo "[✓] Subdomínios brutos: $outfile"
  echo "[✓] Subdomínios únicos: $cleanfile"
  echo "[✓] Subdomínios ativos: $alivefile"
}



bruteAlltxt() {
  wordlist="$HOME/Lists/all.txt"
  resolvers="$HOME/Lists/resolvers.txt"
  outfile="brutesubs_out.txt"
  cleanfile="clean.subdomains"

  [ ! -f "$wordlist" ] && echo "Wordlist não encontrada." && return 1
  [ ! -f "$resolvers" ] && echo "Resolvers não encontrado." && return 1
  [ ! -f domains ] && echo "Arquivo 'domains' não encontrado." && return 1
  ...
  for domain in $(cat domains); do
    echo "[*] Brute forcing: $domain"
    shuffledns -d "$domain" -r "$resolvers" -w "$wordlist"  -mode bruteforce -t 50 -o tmp.out
    anew -i tmp.out "$outfile"
    anew -i tmp.out "$cleanfile"
    rm -f tmp.out
  done
